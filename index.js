  ...
  try {
    ...
    for (const walkDoc of walksSnapshot.docs) {
      try {
        ...
        // Напоминание о предстоящей прогулке (только для прогулок сегодня или регулярных)
        if (walk.date === today || walk.type === "regular") {
          // КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: Проверяем точно 15 минут вместо диапазона
          if (timeToWalkMinutes === 15) {
            console.log(
              `Прогулка ${walkId}: отправка напоминания (15 мин до начала)`
            );
...
`;
            ...
            console.log(`Напоминание о прогулке ${walkId} отправлено`);
          }
        }
      } catch (walkError) {
